...
export const getProfileRouteHandler = (req, res) => {
  ...
  const stringId = req.user.User_PublicID;
  ...
  //     attributes: {
  //       name: meUser.User_Name,
  //       email: meUser.User_Email,
  //       profile_image: null,
  //       createdAt: meUser.User_TimeJoined,
  //       updateAt: meUser.User_TimeJoined
  //     },
  //     links: {
  ...
  const sentData = {
    data: {
      type: 'User',
      id: intId === 1 ? intId : meUser.User_PublicID,
      userInfo: meUser,
      links: {
        self: `${process.env.APP_URL_API}/users/${meUser.id}`
      }
...
export const patchProfileRouteHandler = async (req, res) => {
  ...
  const foundUser = await userModel.findOne({ User_email: currentDataOfUser.email});
  ...
  if (!foundUser) {
    ...
    if (newPassword && newPassword < 7 || newPassword != confirmPassword) {
      ...
      try{
        await userModel.updateOne( { User_Email: foundUser.User_Email }, { $set :{ "User_Name": name, "User_Email": email, "User_Password": hashPassword } });
      } catch(err) {
        console.error(err);
      }
      const sentData = {
      ...
      try {
        await userModel.updateOne( { User_Email: foundUser.User_Email }, { $set :{ "User_Name": name, "User_Email": email } });
      } catch(err) {
        console.error(err);
      }
      const sentData = {
